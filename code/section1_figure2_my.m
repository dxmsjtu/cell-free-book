...
numberOfAntennas = [1 64]; antennaGains = [8 1];
%APs per dimension
nbrAPsPerDim = sqrt(nbrAPs);
%Set the cell edge SNR to 0 dB) in case (a), which becomes -9 dB (1/8) when not accounting for the antenn gain
SNRedge = 1/8;
...
UEpositions = zeros(K,nbrAPs); perBS = zeros(nbrAPs,1);
