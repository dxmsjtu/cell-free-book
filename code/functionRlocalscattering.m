...
%[1] "Foundations of User-Centric Cell-Free Massive MIMO", Foundations and Trends in Signal Processing: Vol. 14: No. 3-4,
...
for column = 2:N
    ...
    if (ASD_theta>0) && (ASD_varphi>0)
        %Define integrand of (2.18) of [1]
        F = @(delta,epsilon)exp(1i*2*pi*distance*sin(varphi+delta).*cos(theta+epsilon)).*...
            exp(-delta.^2/(2*ASD_varphi^2))/(sqrt(2*pi)*ASD_varphi).*...
            exp(-epsilon.^2/(2*ASD_theta^2))/(sqrt(2*pi)*ASD_theta); % f(varphi,theta) c.f. (2.19) of [1] 
        %Compute the integral in (2.18) of [1] by including 20 standard deviations
...
%Compute the spatial correlation matrix by utilizing the Toeplitz structure and scale it properly
R = toeplitz(firstRow);
R = R*(N/trace(R));
